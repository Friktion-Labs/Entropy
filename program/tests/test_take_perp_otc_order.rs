...
use mango::{
    matching::Side,
    state::{AssetType, MangoAccount, MangoGroup, OtcOrderStatus, OtcOrders, UserActiveAssets},
};
...
async fn success_ask() {
    ...
    assert_eq!(creator_mango_account_before.perp_accounts[0].taker_base, 0);
    assert_eq!(creator_mango_account_before.perp_accounts[0].taker_quote, 0);
    assert_eq!(creator_mango_account_before.perp_accounts[0].base_position, 0);
    assert_eq!(creator_mango_account_before.perp_accounts[0].quote_position, 0);
    assert_eq!(counterparty_mango_account_before.perp_accounts[0].base_position, 0);
    assert_eq!(counterparty_mango_account_before.perp_accounts[0].quote_position, 0);
    assert_eq!(counterparty_mango_account_before.perp_accounts[0].taker_base, 0);
    assert_eq!(counterparty_mango_account_before.perp_accounts[0].taker_quote, 0);
    ...
    assert_eq!(creator_mango_account_after.perp_accounts[0].taker_base, 0);
    assert_eq!(creator_mango_account_after.perp_accounts[0].taker_quote, 0);
    assert_eq!(creator_mango_account_after.perp_accounts[0].base_position, -(size as i64));
    assert_eq!(creator_mango_account_after.perp_accounts[0].quote_position, size * 10);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].base_position, size as i64);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].quote_position, size as i64 * -10);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].taker_base, 0);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].taker_quote, 0);

    let counterparty_health = test
        .with_health(
            &mango_group,
            &counterparty_mango_account_after,
            UserActiveAssets::new(
                &mango_group,
                &counterparty_mango_account_after,
        ...
        .await;
    let creator_health = test
        .with_health(
            &mango_group,
            &creator_mango_account_after,
            UserActiveAssets::new(
                &mango_group,
                &creator_mango_account_after,
                vec![(AssetType::Perp, 0)],
    ...
    assert_eq!(counterparty_health, counterparty_deposit_amount - size * 10);
    assert_eq!(creator_health, creator_deposit_amount + size * 10);
...
async fn success_bid() {
    ...
    assert_eq!(creator_mango_account_after.perp_accounts[0].taker_quote, 0);
    assert_eq!(creator_mango_account_after.perp_accounts[0].base_position, size as i64);
    assert_eq!(creator_mango_account_after.perp_accounts[0].quote_position, (size as i64) * -10);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].base_position, -(size as i64));
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].quote_position, size * 10);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].taker_base, 0);
    assert_eq!(counterparty_mango_account_after.perp_accounts[0].taker_quote, 0);
...
async fn success_open_orders() {
    ...
    // Create `MangoAccount` for creator
    let account0_pk = test.create_mango_account(mango_group_pk, 0, 0, None).await;
    let mut packed_creator_open_orders = Vec::new();

    // Create creator open orders
    for n in 0..1 {
        let pk = test
        ...
        packed_creator_open_orders.push(pk);
    }

    // Create `MangoAccount` for counterparty
    let account1_pk = test.create_mango_account(mango_group_pk, 1, 0, None).await;
    let mut packed_counterparty_open_orders = Vec::new();

    // Create counterparty open orders
    for n in 0..1 {
        let pk = test
            .create_spot_open_orders(
                mango_group_pk,
        ...
        packed_counterparty_open_orders.push(pk);
    }
    ...
    test.take_perp_otc_order(
        ...
        &mango_group_cookie.perp_markets[0].address,
        &mango_group.mango_cache,
        &packed_counterparty_open_orders,
        &packed_creator_open_orders,
        1,
        0,
    )
    .await
...
#[tokio::test]
async fn fail_insufficient_funds_margin() {
    let config = MangoProgramTestConfig::default_two_mints();
    let mut test = MangoProgramTest::start_new(&config).await;
    ...
    // Deposit funds
    let creator_deposit_amount = 0;
    let counterparty_deposit_amount = 1000 * (test.quote_mint.unit as u64);
...
#[tokio::test]
async fn fail_insufficient_funds_counterparty() {
    let config = MangoProgramTestConfig::default_two_mints();
    let mut test = MangoProgramTest::start_new(&config).await;
