...
async fn fail_invalid_otc_order_index() {
    ...
    let error =
        test.cancel_perp_otc_order(mango_group_pk, &account0_pk, 0, 1337).await.unwrap_err();
...
async fn fail_invalid_otc_order_status() {
    ...
    let error = test.cancel_perp_otc_order(mango_group_pk, &account0_pk, 0, 0).await.unwrap_err();
    let error_code = get_error_code(error);
