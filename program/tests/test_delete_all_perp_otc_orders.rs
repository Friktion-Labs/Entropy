...
async fn success() {
    ...
    test.cancel_perp_otc_order(mango_group_pk, &account0_pk, 0, 1).await.unwrap();

    test.context.warp_to_slot(9).unwrap();

    test.delete_all_perp_otc_orders(mango_group_pk, &account0_pk, 0).await.unwrap();
...
async fn fail_invalid_otc_order_status() {
    ...
    let error = test.delete_all_perp_otc_orders(mango_group_pk, &account0_pk, 0).await.unwrap_err();
