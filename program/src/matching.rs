...
impl BookSide {
    ...
    pub fn load_mut_checked<'a>(
        ...
        match DataType::try_from(state.meta_data.data_type).unwrap() {
            DataType::Bids => check!(account.key == &perp_market.bids, EntropyErrorCode::Default)?,
            DataType::Asks => check!(account.key == &perp_market.asks, EntropyErrorCode::Default)?,
            _ => return Err(throw!()),
        }
...
impl<'a> Book<'a> {
    pub fn load_checked(
        ...
        check!(bids_ai.key == &perp_market.bids, EntropyErrorCode::InvalidAccount)?;
        check!(asks_ai.key == &perp_market.asks, EntropyErrorCode::InvalidAccount)?;
        Ok(Self {
    ...
    fn new_bid(
        ...
        while rem_quantity > 0 {
            ...
            if price < best_ask_price {
                ...
                return Ok(()); // return silently to not fail other instructions in tx
                               // return Err(throw_err!(EntropyErrorCode::PostOnly));
            }
        ...
        if rem_quantity > 0 && post_allowed {
            if self.bids.is_full() {
                ...
                check!(price > min_bid.price(), EntropyErrorCode::OutOfSpace)?;
            ...
            let owner_slot = mango_account
                .next_order_slot()
                .ok_or(throw_err!(EntropyErrorCode::TooManyOpenOrders))?;
    ...
    pub fn new_ask(
        ...
        if rem_quantity > 0 && post_allowed {
            if self.asks.is_full() {
                ...
                check!(price < max_ask.price(), EntropyErrorCode::OutOfSpace)?;
                let event = OutEvent::new(
                    Side::Ask,
                    max_ask.owner_slot,
                    now_ts,
    ...
    pub fn cancel_order(&mut self, order_id: i128, side: Side) -> MangoResult<LeafNode> {
        match side {
            Side::Bid => {
                self.bids.remove_by_key(order_id).ok_or(throw_err!(EntropyErrorCode::InvalidOrderId))
            }
            Side::Ask => {
                self.asks.remove_by_key(order_id).ok_or(throw_err!(EntropyErrorCode::InvalidOrderId))
            }
        }
    }
